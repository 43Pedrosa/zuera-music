...
export default function PlayerPage() {
  ...
  if (!currentTrack) {
    return <p className="text-muted">Nenhuma música selecionada. Toque uma faixa na home para começar.</p>;
  }

  return (
    <section className="mx-auto max-w-xl space-y-6">
      <div className="relative mx-auto aspect-square w-full max-w-md overflow-hidden rounded-3xl shadow-neon">
        <Image src={currentTrack.artwork?.['1000x1000'] || '/icon.svg'} alt={currentTrack.title} fill className="object-cover" />
      </div>
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold">{currentTrack.title}</h1>
          <p className="text-muted">{currentTrack.user?.name}</p>
        </div>
